...
class CIcoinHooks : public CHooks
{
public:
...
CHooks* InitHook()
{
    ...
    printf("Setup icoin genesis block %s\n", hashGenesisBlock.GetHex().c_str());
    return new CIcoinHooks();
}

bool CIcoinHooks::IsStandard(const CScript& scriptPubKey)
{
    return true;
}
...
void CIcoinHooks::AddToWallet(CWalletTx& wtx)
{
}

bool CIcoinHooks::IsMine(const CTransaction& tx)
{
...
bool CIcoinHooks::IsMine(const CTransaction& tx, const CTxOut& txout)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return false;
...
void CIcoinHooks::AcceptToMemoryPool(CTxDB& txdb, const CTransaction& tx)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return;
...
bool GetNameOfTx(const CTransaction& tx, vector<unsigned char>& name)
{
    ...
    if (!good)
        return error("GetNameOfTx() : could not decode a icoin tx");
...
bool IsConflictedTx(CTxDB& txdb, const CTransaction& tx, vector<unsigned char>& name)
{
    ...
    bool good = DecodeNameTx(tx, op, nOut, vvchArgs);
    if (!good)
        return error("IsConflictedTx() : could not decode a icoin tx");
    int nPrevHeight;
    int nDepth;
    int64 nNetFee;
...
bool CIcoinHooks::ConnectInputs(CTxDB& txdb,
        map<uint256, CTxIndex>& mapTestPool,
        const CTransaction& tx,
        vector<CTransaction>& vTxPrev,
        vector<CTxIndex>& vTxindex,
        ...
        bool fMiner)
{
    ...
    if (tx.nVersion != NAMECOIN_TX_VERSION)
    {
        ...
        // would be lost
        if (found)
            return error("ConnectInputHook() : a non-icoin transaction with a icoin input");
        return true;
    }
    ...
    bool good = DecodeNameTx(tx, op, nOut, vvchArgs);
    if (!good)
        return error("ConnectInputsHook() : could not decode a icoin tx");
    ...
    switch (op)
    {
        case OP_NAME_NEW:
            if (found)
                return error("ConnectInputsHook() : name_new tx pointing to previous icoin tx");
            break;
        case OP_NAME_FIRSTUPDATE:
...
bool CIcoinHooks::DisconnectInputs(CTxDB& txdb,
        const CTransaction& tx,
        CBlockIndex* pindexBlock)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
    ...
    bool good = DecodeNameTx(tx, op, nOut, vvchArgs);
    if (!good)
        return error("DisconnectInputsHook() : could not decode icoin tx");
    if (op == OP_NAME_FIRSTUPDATE || op == OP_NAME_UPDATE)
    {
        CNameDB dbName("cr+", txdb);
...
bool CIcoinHooks::CheckTransaction(const CTransaction& tx)
{
    if (tx.nVersion != NAMECOIN_TX_VERSION)
        return true;
...
bool CIcoinHooks::ExtractAddress(const CScript& script, string& address)
{
...
bool CIcoinHooks::ConnectBlock(CBlock& block, CTxDB& txdb, CBlockIndex* pindex)
{
    return true;
}

bool CIcoinHooks::DisconnectBlock(CBlock& block, CTxDB& txdb, CBlockIndex* pindex)
{
    return true;
}
...
bool CIcoinHooks::GenesisBlock(CBlock& block)
{
    if (fTestNet)
        return false;
...
bool CIcoinHooks::Lockin(int nHeight, uint256 hash)
{
    if (!fTestNet)
